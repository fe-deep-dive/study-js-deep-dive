...
자바스크립트에도 인스턴스 프로퍼티를 클래스 기반 객체지향 언어의 클래스 필드처럼 정의할 수 있는 새로운 표준 사양인 "Class field delcarations"가 TC39 프로세스의 stage 3에 제안되어 있다.
...
- `super()` : 수퍼클래스를 호출하면 constructor(super-constructor)를 호출한다.
- `super.*` : 수퍼클래스를 참조하면 수퍼클래스의 메서드를 호출할 수 있다.
...
2. Derivevd 클래스의 constructor에 전달되고
3. super 호출을 통해
4. Base 클래스의 consturctor에 일부가 전달된다.
...
3. super는 반드시 서브 클래스의 construcor에서만 호출한다.
