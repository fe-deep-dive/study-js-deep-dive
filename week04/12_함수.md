...
원시값은 값에 의한 전달<sup>pass by value</sup>, 개겣는 참조에 의한 전달<sup>pass by reference</sup>방식으로 동작한다.  
...
또한 객체를 불변 객체로 만들어 사용해 마치 원시값처럼 동작하게 하여 객체의 상태 변경이 필요할 경우 깊은 복사를 통해 객체를 생성하고 재할당으로 교체한다.
